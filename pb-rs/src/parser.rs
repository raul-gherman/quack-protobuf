...
fn string(input: &str) -> IResult<&str, String> {
    map(delimited(tag("\""), take_until("\""), tag("\"")), |s: &str| s.to_owned())(input)
}
...
fn syntax(input: &str) -> IResult<&str, Syntax> {
    delimited(
        ...
        alt((value(Syntax::Proto2, tag("\"proto2\"")), value(Syntax::Proto3, tag("\"proto3\"")))),
        pair(many0(br), tag(";"))
    )(input)
}
...
fn package(input: &str) -> IResult<&str, String> {
    delimited(pair(tag("package"), many1(br)), qualifiable_name, pair(many0(br), tag(";")))(input)
}
...
fn extensions(input: &str) -> IResult<&str, Extensions> {
    map(
        delimited(
            ...
            pair(integer, preceded(pair(many0(br), pair(tag("to"), many1(br))), take_until(";"))),
            tag(";")
        ),
        |(from, to)| {
            // TODO: is there a better way to parse "max" or a number?
            let s = to.trim();
            let to = if s == "max" { Extensions::max() } else { s.parse().unwrap() };
            Extensions { from, to }
        }
    )(input)
}
...
fn num_range(input: &str) -> IResult<&str, Vec<i32>> {
    map(separated_pair(integer, tuple((many1(br), tag("to"), many1(br))), integer), |(from_, to)|
        (from_..=to).collect()
    )(input)
}
...
fn map_field(input: &str) -> IResult<&str, (FieldType, FieldType)> {
    delimited(
        ...
        separated_pair(field_type, delimited(many0(br), tag(","), many0(br)), field_type),
        pair(many0(br), tag(">"))
    )(input)
}
            ...
            |(name, events)| {
                ...
                for e in events {
                    match e {
                        ...
                        MessageEvent::ReservedNums(r) => {
                            msg.reserved_nums = Some(r);
                        }
                        MessageEvent::ReservedNames(r) => {
                            msg.reserved_names = Some(r);
                        }
                        ...
                        MessageEvent::Extensions(e) => {
                            msg.extensions = Some(e);
                        }
                        MessageEvent::Ignore => (),
                    }
                }
...
fn enum_field(input: &str) -> IResult<&str, (String, i32)> {
    terminated(
        separated_pair(word, tuple((many0(br), tag("="), many0(br))), alt((hex_integer, integer))),
        pair(
            many0(
                alt((
                    br,
...
pub fn file_descriptor<'a>(
    ...
    let parser = move |
        input: &'a str
    | -> IResult<&'a str, FileDescriptor, nom::error::Error<&str>> {
        map(
            many0(
                alt((
...
mod test {
    ...
    fn test_message() {
        test_syntaxes(
            move |syntax: Syntax| -> Result<(), &str> {
                let msg =
                    r#"message ReferenceData
            {
                ...
                if let ::parser::nom::IResult::Ok((_, mess)) = mess {
    ...
    fn enum_semicolon() {
        ...
        let desc = assert_desc(msg).unwrap();
        assert_eq!(1, desc.messages.len());
        assert_eq!(FieldType::MessageOrEnum("Bar".to_owned()), desc.messages[0].fields[0].typ);
    ...
    fn test_rpc_service() {
        ...
        match file_descriptor(msg) {
            ::parser::nom::IResult::Ok((_, descriptor)) => {
    ...
    fn test_rpc_function() {
        ...
        match rpc_function_declaration(msg) {
            ::parser::nom::IResult::Ok((_, declaration)) => {
    ...
    fn test_word_and_qualifiable_names() {
        fn assert_qualifiable_name_but_not_word(str_to_parse: &str, qualifiable_name_res: &str) {
            ...
            assert_eq!(qualifiable_name_res, qualifiable_name(str_to_parse).unwrap().1);
        }
        ...
        assert_both_ok_same_res("____TestTypes__Repeated_Packed", "____TestTypes__Repeated_Packed");
