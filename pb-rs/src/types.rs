...
impl MessageIndex {
    ...
    fn get_message_mut<'a>(&self, desc: &'a mut FileDescriptor) -> &'a mut Message {
        let first_message = self.indexes.first().and_then(move |i| desc.messages.get_mut(*i));
        self.indexes
            .iter()
            .skip(1)
            .fold(first_message, |cur, next| { cur.and_then(|msg| msg.messages.get_mut(*next)) })
            .expect("Message index not found")
    }
...
impl FieldType {
    ...
    fn wire_type_num(&self, packed: bool) -> u32 {
        if packed { 2 } else { self.wire_type_num_non_packed() }
    }
    ...
    pub fn message(&self) -> Option<&MessageIndex> {
        if let FieldType::Message(ref m) = self { Some(m) } else { None }
    }
...
impl Field {
    ...
    fn write_custom_default<W: Write>(
        ...
        writeln!(w, "    pub const {default_const_name}: {got_type} = {custom_default};")?;
        Ok(())
    }
    ...
    fn write_default<W: Write>(
        ...
        writeln!(w, "            {}: {},", self.name, self.get_field_default(desc, config))?;
        Ok(())
    }
        ...
        match self.frequency.into() {
            GeneratedType::SingularType =>
                match &self.frequency {
                    Frequency::Proto2Frequency(freq) =>
                        match freq {
                            Proto2Frequency::Optional =>
                                writeln!(w, "msg.{} = Some({}),", name, val_cow)?,
                            Proto2Frequency::Required =>
                                writeln!(w, "msg.{} = {},", name, val_cow)?,
                            _ => unreachable!(),
                        }
                    Frequency::Proto3Frequency(freq) =>
                        match freq {
                            Proto3Frequency::Optional =>
                                writeln!(w, "msg.{} = Some({}),", name, val_cow)?,
                ...
                */
                fn get_size_addition(field: &Field, tag_size: usize, s: &str) -> String {
                    format!(
                        ...
                        actual_size = field.typ.get_size(
                            if field.typ.is_fixed_size() {
                                ""
                            } else {
                                s
                            }
                ...
                let conditions_checked = {
                    ...
                    let self_name_size_addition = get_size_addition(
                        self,
                        tag_size,
                    ...
                    match (!self.has_presence(), self.frequency.is_optional()) {
                        (true, true) =>
                            format!(
                                "self.{name}.as_ref().map_or(0, |{maybe_deref_m}| if m != {def} {{ {m_size_addition} }} else {{ 0 }}"
                            ),
                        (true, false) =>
                            format!(
                                "if self.{name} == {def} {{ 0 }} else {{ {self_name_size_addition} }}"
                            ),
                        (false, true) =>
                            format!(
                                "self.{name}.as_ref().map_or(0, |{maybe_deref_m}| {m_size_addition})"
                            ),
                        (false, false) =>
                            get_size_addition(
                                self,
                                tag_size,
                                format!("self.{}", self.name).as_str()
                            ),
                    }
                };
                    ...
                    match self.typ.wire_type_num_non_packed() {
                        ...
                        _ =>
                            writeln!(
                                ...
                                maybe_ampersand = if self.typ.need_to_dereference() {
                                    "&"
                                } else {
                                    ""
                                },
            ...
            GeneratedType::SingularType => {
                ...
                */
                ...
                fn get_write_method(f: &Field, s: &str) -> String {
                    format!("w.write_with_tag({}, |w| w.{})", f.tag(), f.typ.get_write(s, f.boxed))
                }
                ...
                let conditions_checked = {
                    ...
                    let self_name_core = apply_unwrapping_code(
                        self,
                        false,
                        format!("self.{name}").as_str()
                    );
                    ...
                    let self_name_core_equating_cows = apply_unwrapping_code(
                        self,
                        true,
                        format!("self.{name}").as_str()
                    );
                    ...
                    match (!self.has_presence(), self.frequency.is_optional()) {
                        (true, true) =>
                            format!(
                                "self.{name}.as_ref().map_or(Ok(()), |{maybe_deref_m}| if {m_core} != {def} {{ {m_write_method} }} else {{ Ok(()) }})?;"
                            ),
                        (true, false) =>
                            format!(
                                "if {self_name_core_equating_cows} != {def} {{ {self_name_write_method}?; }}"
                            ),
                        (false, true) =>
                            format!(
                                "self.{name}.as_ref().map_or(Ok(()), |{maybe_deref_m}| {m_write_method})?;"
                            ),
...
impl Message {
    fn write_default_consts<W: Write>(
        ...
        // check for that.
        if !self.fields.iter().any(|f| f.can_write_default_const(desc, config)) {
            return Ok(());
        }
        ...
        if !config.dont_use_cow {
            if
                messages
                    ...
                    .any(|m| {
                        m.all_fields().any(
                            |f| (f.typ.has_cow() || (f.packed() && f.typ.is_fixed_size()))
                        )
                    })
            {
                if config.nostd {
            ...
            config.nostd &&
            messages.iter().any(|m| m.all_fields().any(|f| f.typ.has_bytes_and_string()))
        {
    ...
    fn must_generate_getters(&self, desc: &FileDescriptor, config: &Config) -> bool {
        self.fields.iter().any(|f| f.must_generate_getter(desc, config)) && config.generate_getters
    }
    ...
    fn write_definition<W: Write>(
        ...
        writeln!(w, "#[derive({}Debug, PartialEq, Clone)]", custom_struct_derive)?;
    ...
    fn sanity_checks(&self, desc: &FileDescriptor) -> Result<()> {
        for f in self.all_fields() {
            // check reserved
            if
                self.reserved_names.as_ref().map_or(false, |names| names.contains(&f.name)) ||
...
impl Enumerator {
    fn set_package(&mut self, package: &str, module: &str) {
        self.package = package.to_string();
        self.module = module.to_string();
        self.partially_qualified_fields = self.fields
            .iter()
            .map(|f| (format!("{}::{}", &self.name, f.0), f.1))
            .collect();
        self.fully_qualified_fields = self.partially_qualified_fields
            .iter()
            .map(|pqf| {
...
impl OneOf {
    pub fn write_default<W: Write>(&self, w: &mut W, desc: &FileDescriptor) -> Result<()> {
        writeln!(w, "            {}: {},", self.name, self.write_impl_default_field(desc))?;
        Ok(())
    }
    ...
    fn has_lifetime(&self, desc: &FileDescriptor, config: &Config) -> bool {
        self.fields
            .iter()
            .any(|f| {
                f.typ.has_lifetime(desc, config, f.packed(), &mut Vec::new()) &&
                    (!f.deprecated || config.add_deprecated_fields)
            })
    }
    ...
    fn fetch_imports(&mut self, in_file: &Path, import_search_path: &[PathBuf]) -> Result<()> {
        ...
        for import in &self.import_paths {
            ...
            if matching_file.is_none() {
                return Err(
                    Error::InvalidImport(
                        format!("file {} not found on import path", import.display())
                    )
                );
            }
